...
begin
  ...
  if ComboBox3.Text='Не рассмотрено' then begin
    dxDateEdit4.Visible:=False;
    LabelMaskEdit4.Visible:=False;
  end;
  if ComboBox3.Text='Взыскать' then begin
...
begin
  if MessageDlg('ВОССТАНОВИТЬ. Вы уверены?', mtConfirmation, [mbYes, mbNo], 0)=mrYes then
  try
    Screen.Cursor:=crSQLWait;
    with DBForm do begin
      with IBStoredProc do begin
        ...
        ParamByName('USER_UKEY').AsInteger:=User_UKey;
        ExecProc;
        MessageDlg('Данные успешно восстановлены.',mtInformation,[mbOk],0);
...
// --- обновление
    ...
    MessageDlg('ОШИБКА! Данные не восстановлены.',mtError,[mbOk],0);
  end;
  Screen.Cursor:=crDefault;
end;
  ...
  try
    with DBForm.qJurist do begin
      ...
      if RecordCount=1 then begin
        MessageDlg('ВНИМАНИЕ! Данные в востановлении не нуждаются.',mtInformation,[mbOk],0);
        dxCheckEdit2.Checked:=False;
...
begin
  ...
  Then
    Label4.Caption:=''
  Else
    Label4.Caption:='Данные изменены';
end;
